...
export function renderVerticalPlayer(container) {
  ...
  function update() {
    ...
    if (!selectedClip) {
      container.innerHTML = `
        <div class="glass-panel" style="padding: 40px 20px; text-align: center; height: 100%; display: flex; flex-direction: column; align-items: center; justify-content: center;">
          <div style="font-size: 36px; margin-bottom: 12px;">📱</div>
          <div style="font-weight: 700; font-size: 16px;">Select a clip from the studio</div>
          <div style="font-size: 13px; color: var(--text-muted); margin-top: 4px;">Click any 9:16 card to inspect and preview with dynamic captions</div>
        </div>
      `;
      return;
    }
