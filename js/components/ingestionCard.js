...
export function renderIngestionCard(container) {
  ...
  function render() {
    ...
    const defaultPreset = activeBrand?.subtitle_preset || 'hormozi';
          ...
          ${
            ...
            <div class="input-row">
              <div class="url-input-wrap">
                ...
                <input type="url" id="youtube-url-input" placeholder="Paste YouTube Video URL (e.g. https://www.youtube.com/watch?v=...)" required>
              </div>
              ...
              <button type="submit" id="btn-submit-job" class="btn btn-primary btn-lg" ${isSubmitting ? 'disabled' : ''}>
                ${isSubmitting ? '<span class="anim-spin">⚡</span> Processing...' : 'Generate Shorts 🚀'}
              </button>
            </div>
          `
              : `
            <div class="dropzone" id="file-dropzone">
              ...
              <div>
                <div style="font-weight: 700; font-size: 15px; margin-bottom: 4px;">
                  ${selectedFile ? selectedFile.name : 'Drag & drop long-form video or click to browse'}
                </div>
                <div style="font-size: 12px; color: var(--text-muted);">Supports MP4, MOV, MKV, AVI (up to 2GB)</div>
              </div>
            ...
            <div style="margin-top: 16px; display: flex; justify-content: flex-end;">
              <button type="submit" class="btn btn-primary btn-lg" ${!selectedFile || isSubmitting ? 'disabled' : ''}>
                ${isSubmitting ? '<span class="anim-spin">⚡</span> Uploading...' : 'Upload & Direct Shorts 🚀'}
              </button>
            </div>
          `
          }
          ...
          <div class="settings-drawer">
            <div class="drawer-header" id="toggle-drawer">
              <div style="display: flex; align-items: center; gap: 8px; font-weight: 700; font-size: 13px; color: var(--text-secondary);">
                ...
                Custom Video Styling & Directing Options
              </div>
            ...
            <div class="drawer-content" id="drawer-content" style="display: none;">
              ...
              <!-- Subtitle Preset -->
              <div class="setting-item">
                <label class="setting-label">Karaoke Subtitle Preset</label>
                <select id="cfg-subtitle-preset">
                  <option value="hormozi" ${defaultPreset === 'hormozi' ? 'selected' : ''}>🔥 Hormozi (Bold Electric Yellow)</option>
                  <option value="clean" ${defaultPreset === 'clean' ? 'selected' : ''}>✨ Clean (Minimal Neon Cyan)</option>
                  <option value="tech" ${defaultPreset === 'tech' ? 'selected' : ''}>⚡ Tech (Monospace Neon Matrix)</option>
                  <option value="beast" ${defaultPreset === 'beast' ? 'selected' : ''}>🏆 Beast (High Impact Gold)</option>
                </select>
              </div>

              <!-- Pacing Mode -->
              <div class="setting-item">
                <label class="setting-label">Pacing Engine</label>
                <select id="cfg-pacing-mode">
                  <option value="snappy" ${defaultPacing === 'snappy' ? 'selected' : ''}>⚡ Snappy (0.18s Breath Buffer)</option>
                  <option value="hyper">🚀 Hyper (0.12s Aggressive Cuts)</option>
                  <option value="natural">🌿 Natural (Preserve Pauses)</option>
                  <option value="cinematic">🎬 Cinematic (Slow Dynamic Zooms)</option>
                </select>
              </div>
              ...
              <div class="setting-item" style="grid-column: 1 / -1; display: grid; grid-template-columns: repeat(auto-fit, minmax(220px, 1fr)); gap: 12px;">
                <div class="toggle-item">
                  <label for="cfg-dead-space" style="font-size: 13px; font-weight: 600; cursor: pointer;">Cut Dead-Air Silence</label>
                  <input type="checkbox" id="cfg-dead-space" checked style="width: auto; accent-color: var(--primary);">
                </div>
                <div class="toggle-item">
                  <label for="cfg-sfx" style="font-size: 13px; font-weight: 600; cursor: pointer;">Micro Foley SFX Cues</label>
                  <input type="checkbox" id="cfg-sfx" style="width: auto; accent-color: var(--primary);">
                </div>
                <div class="toggle-item">
                  <label for="cfg-top-banner" style="font-size: 13px; font-weight: 600; cursor: pointer;">Top Curiosity Banner</label>
              ...
              <div class="setting-item" style="grid-column: 1 / -1;">
                <label class="setting-label">Custom Directing Instructions (Optional)</label>
                <textarea id="cfg-custom-instructions" rows="2" placeholder="e.g. Focus on contrarian business epiphanies, memorable punchlines, and strong 3-second opening hooks..."></textarea>
              </div>
            </div>
          </div>
        </form>
